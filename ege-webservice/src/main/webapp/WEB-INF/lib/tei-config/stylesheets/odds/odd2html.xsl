...
<xsl:stylesheet xmlns:s="http://www.ascc.net/xml/schematron"
		...
		version="1.0" exclude-result-prefixes="xlink dbk rng
						       tei teix s
						       xhtml a edate
						       estr html
						       pantor sch xd xs xsl" extension-element-prefixes="exsl estr edate">
  ...
  <xd:doc type="stylesheet">
    ...
    <xd:cvsId>$Id: odd2html.xsl 7175 2010-02-07 16:58:54Z rahtz $</xd:cvsId>
    ...
    <xsl:for-each select="..//tei:schemaSpec">
      ...
      <xsl:for-each select="tei:classSpec">
        ...
        <xsl:element name="{$tocElement}">
          ...
          <a xmlns="http://www.w3.org/1999/xhtml" class="toclist" href="#{@ident}">
            <xsl:choose>
